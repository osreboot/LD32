...
import com.osreboot.corrogatedfidelity.Main.GameState;
		...
		if(Mouse.isInsideWindow()){//TODO drag or toggle
			if(!menuOpen){
				if(selected == null){
					...
					if(!Mouse.isButtonDown(0)) selected = null;
				}
			}else{
					...
					if(!clicked && Mouse.isButtonDown(0)){
						if(Mouse.getX() > main.getWidth()/8 && Mouse.getX() < main.getWidth()/8*7 && main.getHeight() - Mouse.getY() > main.getHeight()/4 && main.getHeight() - Mouse.getY() < main.getHeight()/8*6){
							for(Corporation c : Corporation.corps){
								if(!c.isFailed() && Mouse.getX() > (main.getWidth()/64*11) + (Corporation.corps.indexOf(c)*main.getWidth()/16) &&
										Mouse.getX() < (main.getWidth()/64*11) + (Corporation.corps.indexOf(c)*main.getWidth()/16) + main.getWidth()/16 &&
										main.getHeight() - Mouse.getY() > (main.getHeight()/64*11) && //TODO multiply by row
										main.getHeight() - Mouse.getY() < (main.getHeight()/64*11) + main.getHeight()/8){
									for(Card card : Card.cards){
										if(card.isOnGround() && card.getX() > main.getWidth()/4*3){
											card.syncToCorporation(c, framework.getObjectiveManager());
										}
									}
								}
							}
			...
			if(KeybindManager.isFunctionTriggering(FunctionType.MENU)){
				if(!keyTriggered) menuOpen = !menuOpen;
				keyTriggered = true;
			}else keyTriggered = false;
			if(Mouse.isButtonDown(0)){
				if(Mouse.getX() > main.getWidth()/16*14 && !clicked && (menuExtension == main.getWidth()/8*6 || menuExtension == 0)) menuOpen = !menuOpen;
				clicked = true;
			}else clicked = false;
		}
	}
